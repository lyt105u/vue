...
def retrain(X, y, feature_names, task_dir, split_value=1.0, save_model=True, model_role='base', penalty="l2", alpha=1.0, max_iter=500):
    ...
    if split_value != '1.0':
        ...
        results = evaluate_model(y_test, y_pred, logistic_reg, X_test)
        results["loss_plot"] = plot_loss(evals_result)
        results["accuracy_plot"] = plot_accuracy(evals_result)
        shap_result = explain_with_shap(logistic_reg, X_test, feature_names)
        results.update(shap_result)
        lime_result = explain_with_lime(logistic_reg, X_test, y_test, feature_names)
