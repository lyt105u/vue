...
def _build_rf(n_estimators=900, max_depth=50, random_state=0, n_jobs=-1):
...
def train_fold(X_train, y_train, X_val,
               n_estimators=900, max_depth=50, random_state=0, n_jobs=-1,
               focus_class=1):
    """
    ...
    rf = _build_rf(n_estimators, max_depth, random_state, n_jobs)
...
def retrain(X, y, feature_names, task_dir,
    ...
    if split_value != '1.0':
        ...
        results = evaluate_model(y_test, y_pred, rf, X_test, focus_class=fc)

        shap_result = explain_with_shap(rf, X_test, feature_names, focus_class=fc)
        results.update(shap_result)

        lime_result = explain_with_lime(rf, X_test, y_test, feature_names, focus_class=fc)
...
# =========================
# Evaluation (binary / multiclass focus-vs-rest)
# =========================
def evaluate_model(y_test, y_pred, model, x_test, focus_class=1):
    """
    - binary：維持你原本輸出格式 + threshold 掃描行為
    - multiclass：focus_class vs rest（輸出仍維持舊格式，前端不必改）
    """
