...
def run_train_xgb():
    ...
    try:
        result = subprocess.run(
            ['python', 'train_xgb.py', file_name, label_column, split_strategy, split_value, model_name],
            # capture_output=True,        # 捕獲標準輸出和標準錯誤
            stdout=subprocess.PIPE,     # 只捕獲標準輸出
            stderr=subprocess.DEVNULL,  # 忽略標準錯誤
            text=True                   # 將輸出轉換為字符串
...
def run_train_lgbm():
    ...
    try:
        result = subprocess.run(
            ['python', 'train_lgbm.py', file_name, label_column, split_strategy, split_value, model_name],
            # capture_output=True,        # 捕獲標準輸出和標準錯誤
            stdout=subprocess.PIPE,     # 只捕獲標準輸出
            stderr=subprocess.DEVNULL,  # 忽略標準錯誤
            text=True                   # 將輸出轉換為字符串
...
def run_train_lr():
    ...
    try:
        result = subprocess.run(
            ['python', 'train_lr.py', file_name, label_column, split_strategy, split_value, model_name],
            # capture_output=True,        # 捕獲標準輸出和標準錯誤
            stdout=subprocess.PIPE,     # 只捕獲標準輸出
            stderr=subprocess.DEVNULL,  # 忽略標準錯誤
            text=True                   # 將輸出轉換為字符串
